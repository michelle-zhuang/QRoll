...
3. Use specialized tools or READ resource traces if available
...
| Recent regression    | `mcp_gitnexus_detect_changes` to see what your changes affect |
...
```
mcp_gitnexus_query({query: "payment validation error"})
...
```
mcp_gitnexus_context({name: "validatePayment"})
...
```
1. mcp_gitnexus_query({query: "payment error handling"})
   → Processes: CheckoutFlow, ErrorHandling
